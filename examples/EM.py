...
def get_variables(data_dim, num_classes, num_examples):
    ...
    mean_vector = np.asmatrix(np.zeros((num_classes,1), dtype=object)) # Vector of mean variables for each class
    precision_vector = np.asmatrix(np.zeros((num_classes,1), dtype=object)) # Vector of inverse covariance matrices for each class
    categorical_vector = np.asmatrix(np.zeros((num_classes,1), dtype=object)) # Vector of categorical distribution parameters for each class
...
def get_objective(partitioned_set, variable_dict, data_dim, num_classes, num_examples):
    ...
    for i in range(num_examples):
        for j in range(num_classes):
                log_normal_matrix[i,j] = (-1/2)*((train_set[i] - variable_dict['mean'][j,0]).T)*variable_dict['precision'][j,0]*(train_set[i] - variable_dict['mean'][j,0])
...
partitioned_set, data_dim, num_classes, num_examples = get_data(train_percent=0.1)
...
print(variable_dict['mean'])
print(variable_dict['precision'])
print(variable_dict['categorical']) 
print(variable_dict['conditional'])
print(len(cvx.Problem(objective).variables()))
#print(dmcp.bcd.is_dmcp(cvx.Problem(objective)))
"""
'''
...
log_categorical_matrix = np.asmatrix(np.zeros((num_classes,1), dtype=object)) # Log categorical variables
for i in range(num_classes):
    precision_log_det_matrix[i] = ((2*np.pi)**(-data_dim/2))*cvx.log_det(precision_vector[i,0])
    print(precision_vector[i,0].shape)
    log_categorical_matrix[i] = cvx.log(categorical_vector[i,0])
...
for i in range(num_examples):
   for j in range(num_classes):
        log_normal_matrix[i,j] = (-1/2)*(train_set[i] - mean_vector[j,0]).T*precision_vector[j,0]*(train_set[i] - mean_vector[j,0])
        log_probability_matrix[i,j] = cvx.log(probability_matrix[i,j])

objective_array = []
for i in range(num_examples):
    for j in range(num_classes):
        objective_array.append(probability_matrix[i,j]*(precision_log_det_matrix[j,0] + log_normal_matrix[i,j] + log_categorical_matrix[j,0] - log_probability_matrix[i,j]))
objective = cvx.Maximize(sum(objective_array))
#print(dmcp.bcd.is_dmcp(cvx.Problem(objective)))
print(mean_vector)
