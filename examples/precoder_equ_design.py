...
for value in sigma_e_value:
    ...
    for t in range(test_t):
        ...
        r = B*(C*A*s+noise)
        MSE[-1] += cvx.square(cvx.norm(s-r)).value/test_t
        BER[-1] += cvx.sum(cvx.abs(s-r).value>=0.5).value/float(test_t)
    ...
    for t in range(test_t):
        ...
        noise = np.random.randn(m,1)*value
        r = B*(C*A*s+noise)
        MSE_lin[-1] += cvx.square(cvx.norm(s-r)).value/test_t
        BER_lin[-1] += cvx.sum(cvx.abs(s-r).value>=0.5).value/float(test_t)
    ...
    for t in range(test_t):
        ...
        noise = np.random.randn(m,1)*value
        r = B*(C*A*s+noise)
        MSE_noprox[-1] += cvx.square(cvx.norm(s-r)).value/test_t
        BER_noprox[-1] += cvx.sum(cvx.abs(s-r).value>=0.5).value/float(test_t)
